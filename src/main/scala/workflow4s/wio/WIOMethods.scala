...
trait WIOMethods[Ctx <: WorkflowContext, -In, +Err, +Out <: Ctx#State] { self: WIO[In, Err, Out, Ctx] =>
  def flatMap[Err1 >: Err, Out1 <: Ctx#State](f: Out => WIO[Out, Err1, Out1, Ctx])(implicit
      errorCt: ErrorMeta[Err1],
  ): WIO[In, Err1, Out1, Ctx] = WIO.FlatMap(this, f, errorCt)

  def map[Out1 <: Ctx#State](f: Out => Out1): WIO[In, Err, Out1, Ctx] = WIO.Map(
    this,
    identity[In],
    (_: In, out: Out) => f(out),
  )
...
//    ): WIO[Err, O1, StIn, StOut] = ???
  ...
  def transform[NewIn, NewOut <: Ctx#State](f: NewIn => In, g: (NewIn, Out) => NewOut): WIO[NewIn, Err, NewOut, Ctx] =
  ...
  def transformOutput[NewOut <: Ctx#State, In1 <: In](f: (In1, Out) => NewOut): WIO[In1, Err, NewOut, Ctx] = transform(identity, f)
  ...
  def handleErrorWith[Err1, Out1 >: Out <: Ctx#State, ErrIn >: Err, In0 <: In, In1 >: In0](
  ...
  def andThen[Err1 >: Err, Out1 <: Ctx#State](next: WIO[Out, Err1, Out1, Ctx]): WIO[In, Err1, Out1, Ctx] = WIO.AndThen(this, next)

  @targetName("andThenOp")
  def >>>[Err1 >: Err, Out1 <: Ctx#State](next: WIO[Out, Err1, Out1, Ctx]): WIO[In, Err1, Out1, Ctx] = andThen(next)
