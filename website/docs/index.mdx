...
we can plug state, events, and effect type into our context
...
Let’s now see how to run the workflow. To do that, we need a [runtime](runtimes). For simplicity’s sake, we will use
the in-memory synchronized runtime.
