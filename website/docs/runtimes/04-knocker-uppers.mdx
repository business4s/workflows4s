...
<table>
    ...
    <tr>
        <td><code>PekkoKnockerUpper</code></td>
        <td>Pekko-native, `Future`-based. Uses Pekko's `Scheduler.scheduleOnce`. Non-persistent.</td>
        <td>
            <ul>
                <li>Pekko-based ephemeral deployments.</li>
                <li>Avoiding cats-effect dependency.</li>
            </ul>
        </td>
        <td>
            <ul>
                <li>Persistent or distributed setups.</li>
                <li>Non-Pekko deployments.</li>
            </ul>
        </td>
    </tr>
    <tr>
        <td><code>SleepingKnockerUpper</code></td>
        <td>In-memory implementation based on cats-effect `Async[F].sleep`, non-persistent.</td>
        <td>
            <ul>
...
<SbtDependency moduleName={"workflows4s-filesystem"}/>
...
Uses Pekko's native scheduler.

<SbtDependency moduleName={"workflows4s-pekko"}/>

```scala file=./main/scala/workflows4s/example/docs/wakeups/PekkoKnockerUpperExample.scala start=docs_start end=docs_end
```

### QuartzKnockerUpper

<SbtDependency moduleName={"workflows4s-quartz"}/>
