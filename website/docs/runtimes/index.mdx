...
<table>
    ...
    <tr>
        <td><a href="in-memory"><code>InMemoryConcurrentRuntime</code></a></td>
        <td>Fiber-safe, cancellation-safe. Uses cats-effect primitives (Ref, Semaphore). No persistence.</td>
        <td>
            <ul>
    ...
    <tr>
        <td><a href="in-memory"><code>InMemorySynchronizedRuntime</code></a></td>
        <td>Thread-safe via JVM synchronization. No cats-effect dependency. No persistence.</td>
        <td>
            <ul>
