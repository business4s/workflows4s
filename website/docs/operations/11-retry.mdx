...
When drafting workflows that need retry handling, use `WIO.draft.retry()`:

```scala file=./main/scala/workflows4s/example/docs/DraftRetryExample.scala start=start_draft end=end_draft
```
<OperationOutputs name="draft-retry"/>

The draft retry operation simplifies the API by:
- Not requiring explicit retry logic implementation
