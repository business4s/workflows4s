...
Stateless retries are simple and easy to use. They allow rescheduling the wakeup but without any state tracking like attempt counts,
elapsed time or any other information about previous executions.
...
For more complex retry logic, you can use stateful retries,
where the state is persisted through events, same as workflow state.

Moreover,
this approach allows you to recover from errors because it produces events and events can affect the workflow state.

```scala file=./main/scala/workflows4s/example/docs/RetryExample.scala start=start_doc_stateful_full end=end_doc_stateful_full
```
