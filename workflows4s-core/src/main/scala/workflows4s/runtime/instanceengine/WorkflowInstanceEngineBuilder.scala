...
object WorkflowInstanceEngineBuilder {
  ...
  def withJavaTime(clock: Clock = Clock.systemUTC()) = Step1(new BasicJavaTimeEngine[IO](clock))
  def withCatsTime(clock: cats.effect.Clock[IO])     = Step1(new BasicCatsTimeEngine[IO](clock))

  class Step1(val get: WorkflowInstanceEngine[IO]) {
    ...
    class Step2(val get: WorkflowInstanceEngine[IO]) {
