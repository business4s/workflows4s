...
class InMemorySyncWorkflowInstance[Ctx <: WorkflowContext](
  ...
  override protected def updateState(newState: ActiveWorkflow[IO, Ctx]): Id[Unit] = wf = newState

  override protected def lockState[T](update: ActiveWorkflow[IO, Ctx] => Id[T]): Id[T] = lock.synchronized { update(wf) }

  private def updateState(workflow: workflows4s.wio.ActiveWorkflow[IO, Ctx], _events: Seq[WCEvent[Ctx]]): Unit = {
    events ++= _events
    wf = workflow
  }
