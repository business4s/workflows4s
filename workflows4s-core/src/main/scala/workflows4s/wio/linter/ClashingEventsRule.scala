...
object ClashingEventsRule extends Rule {
  ...
  private class ExpectedEventsVisitor[Ctx <: WorkflowContext, In, Err, Out <: WCState[Ctx]](
    ...
    override def onSignal[Sig, Evt, Resp](wio: WIO.HandleSignal[Ctx, In, Out, Err, Sig, Resp, Evt]): List[ClassTag[?]]                     =
      extractMatchedClass(wio.evtHandler)
    override def onRunIO[Evt](wio: WIO.RunIO[Ctx, In, Err, Out, Evt]): List[ClassTag[?]]                                                   =
      extractMatchedClass(wio.evtHandler)
    override def onNoop(wio: WIO.End[Ctx]): List[ClassTag[?]]                                                                              = Nil
    override def onPure(wio: WIO.Pure[Ctx, In, Err, Out]): List[ClassTag[?]]                                                               = Nil
    ...
    override def onAwaitingTime(wio: WIO.AwaitingTime[Ctx, In, Err, Out]): List[ClassTag[?]]                                               =
      extractMatchedClass(wio.releasedEventHandler)
    override def onExecuted[In1](wio: WIO.Executed[Ctx, Err, Out, In1]): List[ClassTag[?]]                                                 = Nil
    override def onDiscarded[In1](wio: WIO.Discarded[Ctx, In1]): List[ClassTag[?]]                                                         = Nil
    override def onRecovery[Evt](wio: WIO.Recovery[Ctx, In, Err, Out, Evt]): List[ClassTag[?]]                                             =
      extractMatchedClass(wio.eventHandler)
    override def onFlatMap[Out1 <: WCState[Ctx], Err1 <: Err](wio: WIO.FlatMap[Ctx, Err1, Err, Out1, Out, In]): List[ClassTag[?]]          = recurse(wio.base)
    override def onHandleError[ErrIn, TempOut <: WCState[Ctx]](wio: WIO.HandleError[Ctx, In, Err, Out, ErrIn, TempOut]): List[ClassTag[?]] = recurse(wio.base)
    override def onRetry(wio: WIO.Retry[Ctx, In, Err, Out]): List[ClassTag[?]]                                                             = recurse(wio.base)
    override def onTransform[In1, Out1 <: WCState[Ctx], Err1](wio: WIO.Transform[Ctx, In1, Err1, Out1, In, Out, Err]): List[ClassTag[?]]   = recurse(
      wio.base,
    )
    override def onHandleErrorWith[ErrIn](wio: WIO.HandleErrorWith[Ctx, In, ErrIn, Out, Err]): List[ClassTag[?]]                           =
    ...
    override def onCheckpoint[Evt, Out1 <: Out](wio: WIO.Checkpoint[Ctx, In, Err, Out1, Evt]): List[ClassTag[?]]                           =
