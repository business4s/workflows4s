...
object ClashingEventsRule extends Rule {
  ...
  private class ClashingEventsVisitor[
      Ctx <: WorkflowContext,
      In,
      Err,
      Out <: WCState[Ctx]
    ...
    override def onSignal[Sig, Evt, Resp](
        wio: WIO.HandleSignal[Ctx, In, Out, Err, Sig, Resp, Evt]
    ): List[LinterIssue] = Nil
    override def onRunIO[Evt](
        wio: WIO.RunIO[Ctx, In, Err, Out, Evt]
    ): List[LinterIssue] = Nil
    override def onNoop(wio: WIO.End[Ctx]): List[LinterIssue] = Nil
    override def onPure(wio: WIO.Pure[Ctx, In, Err, Out]): List[LinterIssue] =
      Nil
    override def onTimer(wio: WIO.Timer[Ctx, In, Err, Out]): List[LinterIssue] =
      Nil
    override def onAwaitingTime(
        wio: WIO.AwaitingTime[Ctx, In, Err, Out]
    ): List[LinterIssue] = Nil
    override def onExecuted[In1](
        wio: WIO.Executed[Ctx, Err, Out, In1]
    ): List[LinterIssue] = Nil
    override def onDiscarded[In1](
        wio: WIO.Discarded[Ctx, In1]
    ): List[LinterIssue] = Nil
    override def onRecovery[Evt](
        wio: WIO.Recovery[Ctx, In, Err, Out, Evt]
    ): List[LinterIssue] = Nil

    override def onFlatMap[Out1 <: WCState[Ctx], Err1 <: Err](
        wio: WIO.FlatMap[Ctx, Err1, Err, Out1, Out, In]
    ): List[LinterIssue] =
      recurse(wio.base, "flatMap")
    override def onRetry(wio: WIO.Retry[Ctx, In, Err, Out]): List[LinterIssue] =
      recurse(wio.base, "retry")
    override def onTransform[In1, Out1 <: WCState[Ctx], Err1](
        wio: WIO.Transform[Ctx, In1, Err1, Out1, In, Out, Err]
    ): List[LinterIssue] =
      recurse(wio.base, "transform")
    override def onHandleErrorWith[ErrIn](
        wio: WIO.HandleErrorWith[Ctx, In, ErrIn, Out, Err]
    ): List[LinterIssue] =
      recurse(wio.base, "base") ++ recurse(wio.handleError, "handler")
    override def onLoop[BodyIn <: WCState[Ctx], BodyOut <: WCState[
      Ctx
    ], ReturnIn](
    ...
    override def onFork(wio: WIO.Fork[Ctx, In, Err, Out]): List[LinterIssue] =
      wio.branches.zipWithIndex.flatMap { case (branch, idx) =>
        recurse(branch.wio, s"branch[${branch.name.getOrElse(idx.toString)}]")
      }.toList
    override def onAndThen[Out1 <: WCState[Ctx]](
        wio: WIO.AndThen[Ctx, In, Err, Out1, Out]
    ): List[LinterIssue] =
      recurse(wio.first, "first") ++ recurse(wio.second, "second")
    override def onEmbedded[InnerCtx <: WorkflowContext, InnerOut <: WCState[
      InnerCtx
    ...
    private def extractMatchedClass(
        eh: EventHandler[?, ?, ?, ?]
    ): List[ClassTag[?]] = eh match {
      case te: TypedEventHandler[?, ?, ?, ?] => List(te.matchedClass)
      case _                                 => Nil
    }

    override def onHandleInterruption(
      ...
      clashIssues.toList ++ recurse(wio.base, "base") ++ recurse(
        wio.interruption,
        "interruption"
      )
    }
    ...
    override def onParallel[InterimState <: WCState[Ctx]](
      ...
      clashIssues.toList ++ wio.elements.zipWithIndex.flatMap { case (e, idx) =>
        recurse(e.wio, s"branch[$idx]")
      }.toList
    }
    ...
    override def onForEach[
      ...
      new ClashingEventsVisitor(wio.elemWorkflow, path :+ "forEach").run

    private def recurse(
        nextWio: WIO[?, ?, ?, Ctx],
        name: String
    ): List[LinterIssue] = new ClashingEventsVisitor(nextWio, path :+ name).run
  ...
  private class ExpectedEventsVisitor[
      Ctx <: WorkflowContext,
      In,
      Err,
      Out <: WCState[Ctx]
    ...
    override def onRunIO[Evt](
      ...
      extractMatchedClass(wio.evtHandler)
    override def onNoop(wio: WIO.End[Ctx]): List[ClassTag[?]] = Nil
    override def onPure(wio: WIO.Pure[Ctx, In, Err, Out]): List[ClassTag[?]] =
      Nil
    override def onTimer(wio: WIO.Timer[Ctx, In, Err, Out]): List[ClassTag[?]] =
      extractMatchedClass(wio.startedEventHandler) ++ extractMatchedClass(
        wio.releasedEventHandler
      )
    override def onAwaitingTime(
        wio: WIO.AwaitingTime[Ctx, In, Err, Out]
    ): List[ClassTag[?]] =
    ...
    override def onFlatMap[Out1 <: WCState[Ctx], Err1 <: Err](
        wio: WIO.FlatMap[Ctx, Err1, Err, Out1, Out, In]
    ): List[ClassTag[?]] = recurse(wio.base)
    override def onRetry(wio: WIO.Retry[Ctx, In, Err, Out]): List[ClassTag[?]] =
      recurse(wio.base)
    override def onTransform[In1, Out1 <: WCState[Ctx], Err1](
        wio: WIO.Transform[Ctx, In1, Err1, Out1, In, Out, Err]
    ): List[ClassTag[?]] = recurse(
      wio.base
    )
    override def onHandleErrorWith[ErrIn](
        wio: WIO.HandleErrorWith[Ctx, In, ErrIn, Out, Err]
    ...
    override def onFork(wio: WIO.Fork[Ctx, In, Err, Out]): List[ClassTag[?]] =
      wio.branches.flatMap(b => recurse(b.wio)).toList
    override def onAndThen[Out1 <: WCState[Ctx]](
        wio: WIO.AndThen[Ctx, In, Err, Out1, Out]
    ): List[ClassTag[?]] =
    ...
    private def recurse(nextWio: WIO[?, ?, ?, Ctx]): List[ClassTag[?]] =
      new ExpectedEventsVisitor(nextWio).run
  }
}
