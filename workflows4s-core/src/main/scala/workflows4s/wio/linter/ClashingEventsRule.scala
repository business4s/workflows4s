...
object ClashingEventsRule extends Rule {
  ...
  private class ClashingEventsVisitor[F[_], Ctx <: WorkflowContext, In, Err, Out <: WCState[Ctx]](
    ...
    override def onCheckpoint[Evt, Out1 <: Out](wio: WIO.Checkpoint[F, Ctx, In, Err, Out1, Evt]): List[LinterIssue] = {
    ...
    private def recurse(nextWio: WIO[?, ?, ?, ?, Ctx], name: String): List[LinterIssue] = new ClashingEventsVisitor(nextWio, path :+ name).run

    private def getExpectedEvents(wio: WIO[?, ?, ?, ?, Ctx]): Set[ClassTag[?]] = {
      val visitor = new ExpectedEventsVisitor(wio)
      visitor.run.toSet
    }
  }

  private class ExpectedEventsVisitor[F[_], Ctx <: WorkflowContext, In, Err, Out <: WCState[Ctx]](
      wio: WIO[F, In, Err, Out, Ctx],
  ) extends Visitor[F, Ctx, In, Err, Out](wio) {
    override type Result = List[ClassTag[?]]

    override def onSignal[Sig, Evt, Resp](wio: WIO.HandleSignal[F, Ctx, In, Out, Err, Sig, Resp, Evt]): List[ClassTag[?]]                     =
      extractMatchedClass(wio.evtHandler)
    override def onRunIO[Evt](wio: WIO.RunIO[F, Ctx, In, Err, Out, Evt]): List[ClassTag[?]]                                                   =
      extractMatchedClass(wio.evtHandler)
    override def onNoop(wio: WIO.End[F, Ctx]): List[ClassTag[?]]                                                                              = Nil
    override def onPure(wio: WIO.Pure[F, Ctx, In, Err, Out]): List[ClassTag[?]]                                                               = Nil
    override def onTimer(wio: WIO.Timer[F, Ctx, In, Err, Out]): List[ClassTag[?]]                                                             =
      extractMatchedClass(wio.startedEventHandler) ++ extractMatchedClass(wio.releasedEventHandler)
    override def onAwaitingTime(wio: WIO.AwaitingTime[F, Ctx, In, Err, Out]): List[ClassTag[?]]                                               =
      extractMatchedClass(wio.releasedEventHandler)
    override def onExecuted[In1](wio: WIO.Executed[F, Ctx, Err, Out, In1]): List[ClassTag[?]]                                                 = Nil
    override def onDiscarded[In1](wio: WIO.Discarded[F, Ctx, In1]): List[ClassTag[?]]                                                         = Nil
    override def onRecovery[Evt](wio: WIO.Recovery[F, Ctx, In, Err, Out, Evt]): List[ClassTag[?]]                                             =
      extractMatchedClass(wio.eventHandler)
    override def onFlatMap[Out1 <: WCState[Ctx], Err1 <: Err](wio: WIO.FlatMap[F, Ctx, Err1, Err, Out1, Out, In]): List[ClassTag[?]]          = recurse(wio.base)
    override def onHandleError[ErrIn, TempOut <: WCState[Ctx]](wio: WIO.HandleError[F, Ctx, In, Err, Out, ErrIn, TempOut]): List[ClassTag[?]] = recurse(
      wio.base,
    )
    override def onRetry(wio: WIO.Retry[F, Ctx, In, Err, Out]): List[ClassTag[?]]                                                             = recurse(wio.base)
    override def onTransform[In1, Out1 <: WCState[Ctx], Err1](wio: WIO.Transform[F, Ctx, In1, Err1, Out1, In, Out, Err]): List[ClassTag[?]]   = recurse(
      wio.base,
    )
    override def onHandleErrorWith[ErrIn](wio: WIO.HandleErrorWith[F, Ctx, In, ErrIn, Out, Err]): List[ClassTag[?]]                           =
      recurse(wio.base) ++ recurse(wio.handleError)
    override def onLoop[BodyIn <: WCState[Ctx], BodyOut <: WCState[Ctx], ReturnIn](
        wio: WIO.Loop[F, Ctx, In, Err, Out, BodyIn, BodyOut, ReturnIn],
    ): List[ClassTag[?]]                                                                                                                   = recurse(wio.body) ++ recurse(wio.onRestart)
    override def onFork(wio: WIO.Fork[F, Ctx, In, Err, Out]): List[ClassTag[?]]                                                               = wio.branches.flatMap(b => recurse(b.wio)).toList
    override def onAndThen[Out1 <: WCState[Ctx]](wio: WIO.AndThen[F, Ctx, In, Err, Out1, Out]): List[ClassTag[?]]                             =
      recurse(wio.first) ++ recurse(wio.second)
    override def onEmbedded[InnerCtx <: WorkflowContext, InnerOut <: WCState[InnerCtx], MappingOutput[_ <: WCState[InnerCtx]] <: WCState[Ctx]](
        wio: WIO.Embedded[F, Ctx, In, Err, InnerCtx, InnerOut, MappingOutput],
    ): List[ClassTag[?]]                                                                                                                   = new ExpectedEventsVisitor(wio.inner).run
    override def onHandleInterruption(wio: WIO.HandleInterruption[F, Ctx, In, Err, Out]): List[ClassTag[?]]                                   =
      recurse(wio.base) ++ recurse(wio.interruption)
    override def onParallel[InterimState <: WCState[Ctx]](wio: WIO.Parallel[F, Ctx, In, Err, Out, InterimState]): List[ClassTag[?]]           =
      wio.elements.flatMap(e => recurse(e.wio)).toList
    override def onCheckpoint[Evt, Out1 <: Out](wio: WIO.Checkpoint[F, Ctx, In, Err, Out1, Evt]): List[ClassTag[?]]                           =
    ...
    private def recurse(nextWio: WIO[?, ?, ?, ?, Ctx]): List[ClassTag[?]] = new ExpectedEventsVisitor(nextWio).run
  }
}
