...
object UnnecessaryErrorHandlerRule extends Rule {
  ...
  private class UnnecessaryErrorHandlerVisitor[F[_], Ctx <: WorkflowContext, In, Err, Out <: WCState[Ctx]](
  ...
  private class CanFailVisitor[F[_], Ctx <: WorkflowContext, In, Err, Out <: WCState[Ctx]](wio: WIO[F, In, Err, Out, Ctx])
      extends Visitor[F, Ctx, In, Err, Out](wio) {
    override type Result = Boolean
    override def onSignal[Sig, Evt, Resp](wio: WIO.HandleSignal[F, Ctx, In, Out, Err, Sig, Resp, Evt]): Boolean                     = wio.meta.error.canFail
    override def onRunIO[Evt](wio: WIO.RunIO[F, Ctx, In, Err, Out, Evt]): Boolean                                                   = wio.meta.error.canFail
    override def onPure(wio: WIO.Pure[F, Ctx, In, Err, Out]): Boolean                                                               = wio.meta.error.canFail
    override def onHandleErrorWith[ErrIn](wio: WIO.HandleErrorWith[F, Ctx, In, ErrIn, Out, Err]): Boolean                           = wio.newErrorMeta.canFail
    override def onHandleError[ErrIn, TempOut <: WCState[Ctx]](wio: WIO.HandleError[F, Ctx, In, Err, Out, ErrIn, TempOut]): Boolean =
      wio.newErrorMeta.canFail

    override def onNoop(wio: WIO.End[F, Ctx]): Boolean                                                                            = false
    override def onTimer(wio: WIO.Timer[F, Ctx, In, Err, Out]): Boolean                                                           = false
    override def onAwaitingTime(wio: WIO.AwaitingTime[F, Ctx, In, Err, Out]): Boolean                                             = false
    ...
    override def onRecovery[Evt](wio: WIO.Recovery[F, Ctx, In, Err, Out, Evt]): Boolean                                           = {
    ...
    override def onCheckpoint[Evt, Out1 <: Out](wio: WIO.Checkpoint[F, Ctx, In, Err, Out1, Evt]): Boolean                         = {
      // This will have to be changed when we enable errors from recover events
      recurse(wio.base)
    }
    override def onForEach[Elem, InnerCtx <: WorkflowContext, ElemOut <: WCState[InnerCtx], InterimState <: WCState[Ctx]](
