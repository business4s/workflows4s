...
object ClashingSignalsRule extends Rule {
  ...
  private class ClashingSignalsVisitor[
      Ctx <: WorkflowContext,
      In,
      Err,
      Out <: WCState[Ctx]
    ...
    override def onHandleInterruption(
      ...
      val clashIssues =
        clashes.map(sig =>
          LinterIssue(
            s"Clashing signal: ${sig.name} (${sig.id}) expected in both base and interruption trigger",
            id,
            path
          )
        )
    ...
    override def onParallel[InterimState <: WCState[Ctx]](
      ...
      val clashIssues = for {
        ...
        s"Clashing signal: ${clash.name} (${clash.id}) expected in parallel branches ${i} and ${j}",
        id,
        path
      )
    ...
    ): List[LinterIssue] = recurse(wio.base, "checkpoint")
    ...
    private def getExpectedSignals(
        wio: WIO[?, ?, ?, Ctx]
    ): Set[SignalDef[?, ?]] = {
