...
object BusyLoopRule extends Rule {
  ...
  private class BusyLoopVisitor[Ctx <: WorkflowContext, In, Err, Out <: WCState[Ctx]](
    ...
    private def recurse(nextWio: WIO[IO, ?, ?, ?, Ctx], name: String): List[LinterIssue] = new BusyLoopVisitor(nextWio, path :+ name).run

    private def containsProgressMark(wio: WIO[IO, ?, ?, ?, Ctx]): Boolean = {
      val visitor = new ProgressMarkVisitor(wio)
      visitor.run
    }
  }

  private class ProgressMarkVisitor[Ctx <: WorkflowContext, In, Err, Out <: WCState[Ctx]](wio: WIO[IO, In, Err, Out, Ctx])
      extends Visitor[IO, Ctx, In, Err, Out](wio) {
    override type Result = Boolean
    override def onSignal[Sig, Evt, Resp](wio: WIO.HandleSignal[IO, Ctx, In, Out, Err, Sig, Resp, Evt]): Boolean = true
    override def onAwaitingTime(wio: WIO.AwaitingTime[IO, Ctx, In, Err, Out]): Boolean                           = true
    override def onTimer(wio: WIO.Timer[IO, Ctx, In, Err, Out]): Boolean                                         = true

    override def onRunIO[Evt](wio: WIO.RunIO[IO, Ctx, In, Err, Out, Evt]): Boolean       = false
    override def onNoop(wio: WIO.End[IO, Ctx]): Boolean                                  = false
    override def onPure(wio: WIO.Pure[IO, Ctx, In, Err, Out]): Boolean                   = false
    ...
    override def onRecovery[Evt](wio: WIO.Recovery[IO, Ctx, In, Err, Out, Evt]): Boolean = false

    override def onFlatMap[Out1 <: WCState[Ctx], Err1 <: Err](wio: WIO.FlatMap[IO, Ctx, Err1, Err, Out1, Out, In]): Boolean          = recurse(wio.base)
    override def onHandleError[ErrIn, TempOut <: WCState[Ctx]](wio: WIO.HandleError[IO, Ctx, In, Err, Out, ErrIn, TempOut]): Boolean = recurse(
      wio.base,
    )
    ...
    override def onHandleErrorWith[ErrIn](wio: WIO.HandleErrorWith[IO, Ctx, In, ErrIn, Out, Err]): Boolean                           =
    ...
    override def onCheckpoint[Evt, Out1 <: Out](wio: WIO.Checkpoint[IO, Ctx, In, Err, Out1, Evt]): Boolean                           = recurse(wio.base)
    override def onForEach[Elem, InnerCtx <: WorkflowContext, ElemOut <: WCState[InnerCtx], InterimState <: WCState[Ctx]](
        wio: WIO.ForEach[IO, Ctx, In, Err, Out, Elem, InnerCtx, ElemOut, InterimState],
    ): Boolean                                                                                                                       = new ProgressMarkVisitor(wio.elemWorkflow).run

    private def recurse(nextWio: WIO[IO, ?, ?, ?, Ctx]): Boolean = new ProgressMarkVisitor(nextWio).run
  }
}
