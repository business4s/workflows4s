...
/** Identifies an external signal that a workflow can receive.
  *
  * Instances should be defined as stable values shared between the workflow definition
  * and the code that delivers signals at runtime.
  * The `id` must be stable across deployments as it is persisted in the event journal.
  */
