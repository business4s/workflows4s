...
object RunIOEvaluator {
  ...
  private class RunIOVisitor[Ctx <: WorkflowContext, In, Err, Out <: WCState[
    Ctx,
  ]](
    ...
    def onRunIO[Evt](wio: WIO.RunIO[Ctx, In, Err, Out, Evt]): Result =
      wio.buildIO(input).map(wio.evtHandler.convert).map(_.rightIor).some

    def onFlatMap[Out1 <: WCState[Ctx], Err1 <: Err](
        wio: WIO.FlatMap[Ctx, Err1, Err, Out1, Out, In],
    ): Result = recurse(wio.base, input)
    ...
    def onEmbedded[InnerCtx <: WorkflowContext, InnerOut <: WCState[
      ...
      val newState: WCState[InnerCtx] =
        wio.embedding.unconvertStateUnsafe(lastSeenState)
    ...
    override def onRetry(
        wio: WIO.Retry[Ctx, In, Err, Out],
    ): Option[IO[Ior[Instant, WCEvent[Ctx]]]] = {
          ...
          wio.mode match {
            case Mode.Stateless(errorHandler)               =>
              errorHandler(input, err, lastSeenState, now)
                .flatMap({
                  ...
                  case WIO.Retry.Stateless.Result.ScheduleWakeup(at) =>
                    IO.pure(at.leftIor)
                })
            case Mode.Stateful(errorHandler, _, retryState) =>
              errorHandler(input, err, lastSeenState, retryState).flatMap({
                ...
                case WIO.Retry.Stateful.Result.Recover(event)            =>
                  event.rightIor.pure[IO]
              })
          },
        ),
      )
    ...
    override def onForEach[
      ...
      state.toList
        .collectFirstSome((elemId, elemWio) =>
          new RunIOVisitor(elemWio, input, wio.initialElemState(), now).run
            .tupleLeft(elemId),
        )
        .map { case (elemId, io) =>
          io.map(_.map(wio.eventEmbedding.convertEvent(elemId, _)))
        }
    }
