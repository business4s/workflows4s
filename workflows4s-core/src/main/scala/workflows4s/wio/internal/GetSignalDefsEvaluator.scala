...
private[workflows4s] object GetSignalDefsEvaluator {
  ...
  private class GetSignalDefsVisitor[
      Ctx <: WorkflowContext,
      In,
      Err,
      Out <: WCState[Ctx],
    ...
    ): Result                                                                 = List(wio.sigDef)
    override def onExecuted[In1](
        wio: WIO.Executed[Ctx, Err, Out, In1],
    ): Result                                                                 = Nil
    override def onRunIO[Evt](wio: WIO.RunIO[Ctx, In, Err, Out, Evt]): Result =
      Nil
    ...
    override def onFork(wio: WIO.Fork[Ctx, In, Err, Out]): Result =
      wio.selected.map(idx => recurse(wio.branches(idx).wio)).getOrElse(Nil)
    override def onHandleInterruption(
        wio: WIO.HandleInterruption[Ctx, In, Err, Out],
    ): Result                                                     = recurse(wio.base) ++ recurse(wio.interruption)
