...
private[workflows4s] object GetSignalDefsEvaluator {
  ...
  private class GetSignalDefsVisitor[Ctx <: WorkflowContext, In, Err, Out <: WCState[Ctx]](
    ...
    // Recursive cases
    override def onDiscarded[In1](wio: WIO.Discarded[Ctx, In1]): Result = recurse(wio.original)
    ...
    override def onEmbedded[InnerCtx <: WorkflowContext, InnerOut <: WCState[InnerCtx], MappingOutput[_ <: WCState[InnerCtx]] <: WCState[Ctx]](wio: WIO.Embedded[Ctx, In, Err, InnerCtx, InnerOut, MappingOutput]): Result = GetSignalDefsEvaluator.run(wio.inner)
    override def onAndThen[Out1 <: WCState[Ctx]](wio: WIO.AndThen[Ctx, In, Err, Out1, Out]): Result = recurse(wio.first)
    override def onHandleErrorWith[ErrIn](wio: WIO.HandleErrorWith[Ctx, In, ErrIn, Out, Err]): Result = recurse(wio.base) ++ recurse(wio.handleError)
    override def onLoop[BodyIn <: WCState[Ctx], BodyOut <: WCState[Ctx], ReturnIn](wio: WIO.Loop[Ctx, In, Err, Out, BodyIn, BodyOut, ReturnIn]): Result = recurse(wio.current.wio)
    ...
    override def onParallel[InterimState <: WCState[Ctx]](wio: WIO.Parallel[Ctx, In, Err, Out, InterimState]): Result = wio.elements.flatMap(elem => recurse(elem.wio)).toList
