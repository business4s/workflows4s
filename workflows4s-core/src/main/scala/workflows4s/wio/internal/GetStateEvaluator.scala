...
object GetStateEvaluator {
  ...
  def extractLastState[F[_], Ctx <: WorkflowContext, In](
      wio: WIO[F, In, ?, WCState[Ctx], Ctx],
      input: In,
  ...
  private class GetStateVisitor[F[_], Ctx <: WorkflowContext, In, Err, Out <: WCState[Ctx]](
    ...
    def onDiscarded[In](wio: WIO.Discarded[F, Ctx, In]): Result                                                           = recurse(wio.original, wio.input)
