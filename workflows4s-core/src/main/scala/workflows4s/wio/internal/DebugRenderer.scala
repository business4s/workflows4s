...
object DebugRenderer {
  ...
  private def renderModel(model: WIOExecutionProgress[?]): Description = {
    val tpe                  = model match {
      ...
      case _: WIOExecutionProgress.Recovery[?]      => "Recovery"
      case _: WIOExecutionProgress.Retried[?]       => "Retried"
      case _: WIOExecutionProgress.ForEach[?, ?, ?] => "ForEach"
    }
    val name                 = model match {
      case _: WIOExecutionProgress.Sequence[?]      => None
      case _: WIOExecutionProgress.Dynamic          => None
      case x: WIOExecutionProgress.RunIO[?]         => x.meta.name
      case _: WIOExecutionProgress.HandleError[?]   => None
      case x: WIOExecutionProgress.HandleSignal[?]  => x.meta.operationName
      case _: WIOExecutionProgress.End[?]           => None
      ...
      case _: WIOExecutionProgress.Interruptible[?] => None
    ...
    val description          = model match {
      case _: WIOExecutionProgress.Sequence[?]      => None
      case _: WIOExecutionProgress.Dynamic          => None
      case _: WIOExecutionProgress.RunIO[?]         => None
      case _: WIOExecutionProgress.HandleError[?]   => None
      case x: WIOExecutionProgress.HandleSignal[?]  => s"Signal: ${x.meta.signalName}".some
      case _: WIOExecutionProgress.End[?]           => None
      case _: WIOExecutionProgress.Pure[?]          => None
      case _: WIOExecutionProgress.Loop[?]          => None
      case _: WIOExecutionProgress.Fork[?]          => None
      case _: WIOExecutionProgress.Interruptible[?] => None
