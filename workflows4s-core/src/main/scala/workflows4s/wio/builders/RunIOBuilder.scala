...
object RunIOBuilder {
  ...
  trait Step0[Ctx <: WorkflowContext] {
    ...
    class RunIOBuilderStep1[Input] {
      def apply[Evt <: WCEvent[Ctx]: ClassTag](f: WIOContext[WCState[Ctx]] ?=> Input => WCEffect[Ctx][Evt]): Step2[Input, Evt] = {
        new Step2[Input, Evt](ctx => f(using ctx))
      }

      class Step2[In, Evt <: WCEvent[Ctx]: ClassTag](getIO: WIOContext[WCState[Ctx]] => In => WCEffect[Ctx][Evt]) {
