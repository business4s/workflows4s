...
import workflows4s.wio.{ErrorMeta, WCEffect, WCEvent, WCState, WIO, WorkflowContext}
...
object RunIOBuilder {
  ...
  trait Step0[Ctx <: WorkflowContext] {
    ...
    class RunIOBuilderStep1[Input] {
      def apply[Evt <: WCEvent[Ctx]: ClassTag](f: Input => WCEffect[Ctx][Evt]): Step2[Input, Evt] = {
        new Step2[Input, Evt](f)
      }

      class Step2[In, Evt <: WCEvent[Ctx]: ClassTag](getIO: In => WCEffect[Ctx][Evt]) {
