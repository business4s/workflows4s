...
object BranchBuilder {
  ...
  trait Step0[Ctx <: WorkflowContext]() {
    ...
    case class BranchBuilderStep1[In]() {
      ...
      def when[Err, Out <: WCState[Ctx]](cond: In => Boolean)(wio: WIO[IO, In, Err, Out, Ctx]): Step2[In, Err, Out] =
        Step2(x => Option.when(cond(x))(x), wio, None)

      def create[T, Err, Out <: WCState[Ctx]](cond: In => Option[T])(wio: WIO[IO, T, Err, Out, Ctx]): Step2[T, Err, Out] =
        Step2(cond, wio, None)

      case class Step2[T, Err, Out <: WCState[Ctx]](cond: In => Option[T], wio: WIO[IO, T, Err, Out, Ctx], name: Option[String]) {
        ...
        def done: WIO.Branch[IO, In, Err, Out, Ctx, ?] = WIO.Branch(cond, wio, name)
      }
