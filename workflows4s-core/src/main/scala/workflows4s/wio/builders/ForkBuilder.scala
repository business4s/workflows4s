  ...
  trait Step0[F[_], Ctx <: WorkflowContext]() {
    ...
    case class ForkBuilder[-In, +Err, +Out <: WCState[Ctx]](branches: Vector[WIO.Branch[F, In, Err, Out, Ctx, ?]], name: Option[String]) {
      ...
      def matchCondition[T, Err1 >: Err, Out1 >: Out <: WCState[Ctx], In1 <: In](condition: In1 => Boolean, name: String = null)(
        ...
        Vector(
          WIO.Branch[F, In1, Err1, Out1, Ctx, In1](x => Option.when(condition(x))(x), onTrue, Some("Yes")),
          WIO.Branch[F, In1, Err1, Out1, Ctx, In1](x => Option.when(!condition(x))(x), onFalse, Some("No")),
        ),
        Option(name),
        None,
      )
