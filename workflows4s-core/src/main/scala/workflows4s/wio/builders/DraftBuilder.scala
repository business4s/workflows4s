...
object DraftBuilder {
  ...
  trait Step0[Ctx <: WorkflowContext]() {
    ...
    object DraftBuilderStep1 {
      ...
      def timer(name: String = null, duration: FiniteDuration = null)(using autoName: sourcecode.Name): WIO.Timer[IO, Ctx, Any, Nothing, Nothing] =
        WIO.Timer(
      ...
      def interruptionSignal(
          ...
          error: String = null,
      )(using autoName: sourcecode.Name): WIO.Interruption[IO, Ctx, Nothing, Nothing] = {
        val draftSignalHandling: WIO[IO, WCState[Ctx], Nothing, Nothing, Ctx] = WIO
          ...
          .map(_ => ???)
        WIO.Interruption[IO, Ctx, Nothing, Nothing](draftSignalHandling, WIO.HandleInterruption.InterruptionType.Signal)
      }
      ...
      def interruptionTimeout(
        ...
        val draftTimer: WIO[IO, WCState[Ctx], Nothing, Nothing, Ctx] = WIO
          .Timer(
            Option(duration) match {
              case Some(value) => WIO.Timer.DurationSource.Static(value.toJava)
              case None        => WIO.Timer.DurationSource.Dynamic(_ => ???)
          ...
          .transformInput((_: WCState[Ctx]) => ???)
          .map(_ => ???)
        WIO.Interruption[IO, Ctx, Nothing, Nothing](draftTimer, WIO.HandleInterruption.InterruptionType.Timer)
      }
